...
def main():
    if "-h" in sys.argv[1]:
        ...
        reference = '~/Project/Reference/hg19/Sequence/Chromosomes'
        project_root = '~/Project/SNuPer_results'
        ...
        rna = [(rna_cross.get(f) , int(os.popen(
            "samtools view -q 10 {} chr{}:{} | wc -l".format(f, snp_chrom, region)).read().rstrip())
                ) for f in rna_bams]
...
# updatedb -U ~/Project/SNuPer_results/chr10 -o ~/Project/SNuPer_results/chr10/mlocate.db --require-visibility 0

# TODO command to use locate:
# locate -d ~/Project/SNuPer_results/chr10/mlocate.db 10_15429889:XY.snp
